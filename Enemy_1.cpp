...
void Enemy_1::takeDamage(int damage)// ダメージを受けたときの処理
{
	if (!m_takeDamage) {
		m_takeDamage = true;
		m_frameIndex = 0;
...
RectF Enemy_1::attackRect(const Vec2& cam) const
{
...
RectF Enemy_1::chaseRect(const Vec2& cam) const
{
...
Line Enemy_1::makeGroundProbeLine(const Vec2& cam) const
{
	// 地面探査用の線分を作成
...
void Enemy_1::update(Player& player, Game_Map& map)
{
	...
	const bool inBlockedState = (m_state == AnimState_Enemy1::Hurt) || (m_state == AnimState_Enemy1::Attack);// ダメージ中or攻撃中は行動制限
	...
	else if (m_state == AnimState_Enemy1::Attack) {// 攻撃中
	...
	if (m_mode == Behavior_Enemy1::Attack) {// 攻撃モード
		m_Speed = 0.0;
		isRuning = false;
	...
	if (gotHit) {//
		if (m_state != AnimState_Enemy1::Hurt) {
			setState(AnimState_Enemy1::Hurt);
			m_Speed = 0.0f;
		}
	}
	...
	if (m_state != AnimState_Enemy1::Attack && m_state != AnimState_Enemy1::Hurt) {// 攻撃中・被弾中以外は状態を速度に応じて更新
		setState(isRuning ? AnimState_Enemy1::Run : AnimState_Enemy1::Idle);
	}
}
